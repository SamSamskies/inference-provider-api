...
Demos under `examples/` should load published packages the way real consumers do (pinned CDN ESM imports such as `https://esm.sh/ipa-tools@0.2.0`, jsDelivr, or an import map to the same). That is the point of the demo: show the public package API, not a vendored copy of `packages/*/dist`.
